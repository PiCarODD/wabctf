 ...
 * @Last Modified time: 2019-05-07 13:56:41
**/
-->
...
?>
<!--if logged user type is not admin1337s we cant see add challenge file-->
<h1 style="text-align: center;">Cryptography</h1><?php if($_SESSION['Type']=="admin1337s"){ echo "<center><a href='addchal1337.php'>Add Challenges</a></center>";}?>
<?php
$cat='Cryptography';
...
if(isset($_POST['submit'])){
	$flag=mysqli_real_escape_string($conn,$_POST['flag']);//avoid sql injection
	$chalid=mysqli_real_escape_string($conn,$_POST['cid']);//dont look my sql code before using my site //wabctf0.000webapps.com
	$uid=$_SESSION['name'];
	...
	if(empty($row1)){//is sql data result null?
		if($realflag['flag']==$flag){//compare flag vs user input flag if true add points
